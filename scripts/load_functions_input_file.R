...
# Load_Functions_Input_File.R
# Contains helper functions for processing input data and calculating reservoir operations
# Includes functions for instream flows, rule curves, refill calculations, and flow routing

get_iflow_mainstem = function(station) {
	ifile = read.table(paste0('inputs/iflow_rules/', station, '_iflow_rules', sep=""))
...
calc_refill_curve <- function(type) {
	...
	if (type == "regular") {
		write.table(RefillCurve, "inputs/miscellaneous/test_weekly_assured_before_refill.txt", col.names=T, row.names=F, quote=F)
	} else if (type == "minimum") {
		write.table(RefillCurve, "inputs/miscellaneous/test_weekly_assured_before_refill_min.txt", col.names=T, row.names=F, quote=F)
	}	
	return(RefillCurve)
}
