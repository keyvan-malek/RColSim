# VIC_Data.R
# Reads and processes VIC (Variable Infiltration Capacity) model output data
# Converts climate data to weekly reservoir inflows and other hydrologic inputs
...
VIC_Data <- function() {
	...
	terms <- c("Flow", "Dem", "Iflow", "Curt", "Variable", "Ret", "Residual", "Refill", "OperatingRuleCurve", "ECCurve")
	for (j in terms) {
	...
	terms <- c("OperatingRuleCurve", "FloodCurve")
	for (j in terms) {
		cols <- grep(j, names(input_file))
		for (i in cols) {
			var <- names(input_file)[i]
