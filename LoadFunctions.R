...
MICurFC_Cont <- function() { # December -- June flood curve
	...
	if (month_in_year %in% 3:11) {
		if (DallesRunoffAprAug >= 111.0E06) {
			if (MISumRunoffAprilAug <= 20.0E06) {
				if (month_in_year %in% 3:4) {
					MI_CurFC_o <- 19875520 # Oct-Nov
				  ...
				  MI_CurFC_o <- MIFlood_DecToApr[2,1] # dry year flood curve (others are too low for this late in the season)
				}
			} else { # MISumRunoffAprilAug > 20E6
				MIFlood12 <- MIFlood_712[week_counter_in_year(), 13] # Wet year flood curve
				MI_CurFC_o <- MIFlood12
			}
		  ...
		  if (month_in_year %in% 3:9) {
		    ...
		    MI_CurFC_o <- MIFlood_712[week_counter_in_year(), 2] # Dry year curve
		  }
		}
...
DUCurFC_Cont <- function() {
	...
	if (DURunoffAprAug <= 2.0E06) {
		...
		} else if (month_in_year %in% 7:9) {
			DUCurFC_Cont_o <- DU_JAN_FAMAJ[row_no, 3] # FMAMJ_DU 
		} else if (month_in_year %in% 10:11) {
		  DUCurFC_Cont_o <- DU_JAN_FAMAJ[2,3] 
		} else {
	...
	} else if (DURunoffAprAug > 2.0E6 && month_in_year %in% 10:11) {
...
# For some dams the decision is based on the forecast of  total inflow to dam itself.
# For other dams the flood storage decision is based on the forecast of  total runoff anticipated at the Dalles.
FMAMJ_DU <- function() { # Feb - Jun flood curve
...
HHCurFC_Cont <- function() {
	...
	if (HHSumMaySept <= 3.68E06) {
		...
		} else { # Outside Oct - April flood evacuation period
...
DWCurFC_Cont <- function() {
	...
	} else { # Outside October - April flood evacuation period
	  DWCurFC_Cont_o <- DWFullPoolVol
	}
