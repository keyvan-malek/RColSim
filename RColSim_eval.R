...
sim_reg <- read.table("Historical_baseline/dams_out.txt", header=T, stringsAsFactors=F)
sim_reg[,-c(1:5)] <- sim_reg[,-c(1:5)] / 13.8843
sim_nat <- read.table("~/RColSim_v1/Preliminary/output/Historical_baseline/supply_baseline.txt", header=T) 
  ...
  for (s in stns) {
	...
	if (timestep == "mean_monthly") {
		svg(paste0(plot_dir, flow, "_", timestep, "_", s, ".svg"), width=7, height=5, pointsize=12)
			plot(obs[,s] ~ obs[,1], ylim=c(ymin, ymax), type='l', ylab="Flow (cfs)", xaxt="n", xlab="")
